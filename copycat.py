...
x = fluffy
y = fluffy.copy()
x.setAge(6) # set the age of the og fluffy to 6
print(fluffy.getAge()) # prints 6
print(y.getAge()) # prints 5 because the copy is unchanged

A = [x, y] 
B = A
C = list(A) # shallow copy, not copying the inner elements, only the outer list

D = [] # make a new list, for deep copy, this list will have the inner elements copied.
for cat in A:
    D.append(cat.copy()) 
